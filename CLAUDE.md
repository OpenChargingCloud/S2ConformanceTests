...
Interoperability test suite for the S2 stack in the `libs/WWCP_S2` submodule (S2 JSON v1.0.0
messages, S2 Connect pairing and session initiation), run against the two reference
implementations s2-python (`libs/s2-python`) and s2-rust (`libs/s2-rust`) in both roles and
both directions. Phase 11b of WWCP_S2's PLAN.md, carried here so that the library does not
have to.
...
- **The drivers:** `tools/s2-python-harness/*.py` and the cargo crate `tools/s2-rust-harness/`.
  Child processes speaking JSON lines on stdin/stdout, scripted by the tests through
  `ExternalProcess`; every message that crosses the wire on the WWCP side is validated
  against the embedded schemas by `TrafficRecorder`.
...
Expected (2026-09-05): 110 tests, 95 passed, 15 known issues (warnings), 0 failed.
Prerequisites: .NET 10 SDK, Python 3.9–3.13, Rust. On Windows the S2 Connect drivers build
and run inside WSL (Debian with rustup; `s2energy-connection` is Unix-only), and the
"s2-rust client → WWCP server" tests additionally need the Windows firewall to admit the test
host from the WSL network. Fixtures skip with a reason when a toolchain is missing;
`S2_INTEROP_REQUIRE=1` (what the CI sets) makes that a failure. All `S2_INTEROP_*` variables
...
- Never change wire behaviour of WWCP S2 because a reference implementation differs: check
  the specification text first (quoted in FINDINGS.md). The reference implementations have
  been wrong before (P1–P4, R1–R7).
